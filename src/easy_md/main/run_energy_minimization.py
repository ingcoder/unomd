...
def energy_force_post_simulation(simulation):
    ...
    forces = simulation.context.getState(getForces=True).getForces()  # This is a list of Vec3 objects
    ...
    print(f"Maximum Force Magnitude after Minimization: {top_10_force_magnitudes}")
...
def main(config):
    ...
    # Load files using the flat config structure
    off_top = Topology.from_json(open(config['path_openff_topology']).read())
    omm_system, omm_top, _ = fileparser.load_files(config['path_openmm_system'], config['path_openmm_topology'])
    ...
    save_min_structure(emin_simulation, config['path_emin_structure'], config['path_emin_state'])
